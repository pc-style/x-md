...
Bulk NDJSON imports end with `{"meta":…}` after all post lines. If one timeline page remains unavailable after retries, the import can finish with `meta.warnings: ["page_missing"]`, `truncated: true`, and `floor_reached: false`; retry the range to fill the gap. If it cannot continue, the last line is an RFC 9457 `error` with a retryable `partial_upstream_failure` code and `streamed_posts`. An HTTP 200 on a stream does not by itself mean the import completed.
