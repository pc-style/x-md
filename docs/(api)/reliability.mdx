...
| `404` | Confirmed missing or inaccessible content, or an unknown path | Follow the `links` in the body. Check the public source URL on x.com. An ambiguous provider miss uses `503`. |
...
| `502`, `503` | Provider failure, ambiguous profile miss, or live search has no healthy session | Honor `Retry-After` when present; retry with backoff and jitter. |
...
Bulk NDJSON imports end with `{"meta":…}` after all post lines. If one timeline page remains unavailable after retries, the import can finish with `meta.warnings: ["page_missing"]`, `truncated: true`, and `floor_reached: false`; retry the range to fill the gap. If it cannot continue, the last line is an RFC 9457 `error`. After posts have streamed its code is `partial_upstream_failure` and `streamed_posts` gives their count; earlier failures can use codes such as `upstream_unavailable` or `internal_error`. An HTTP 200 on a stream does not by itself mean the import completed.
