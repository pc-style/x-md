...
| `until` | now | Newest post to include. To continue past a truncated result, send its `meta.oldest` here. |
...
| `format` | `json` | `json` answers once, sorted newest first. `ndjson` streams `{"post": …}` lines as chains deliver them (unsorted), then a final `{"meta": …, "profile": …}` line. |
