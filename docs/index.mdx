...
# An account's last year, replies included, as raw JSON in seconds
curl -sS 'https://x.pcstyle.dev/paulg/posts?since=2025-09-01&max_posts=2000'
```

Lists go up to **100 per page**; pick Latest, Top, Photos, Videos, or Users with `feed`. JSON responses include the rendered `markdown` too. [See response formats](/responses). For a whole account history use the [bulk import](/bulk-import).
...
- You want a public profile's bio and latest posts without holding X credentials.
- You are onboarding a user and need their last 500–2000 posts, replies included, before they finish reading the welcome screen.
...
- **Firehose or multi-account collection.** The service is rate limited and cached for interactive use. One account's history is different: [import it](/bulk-import) instead of paging.
...
<CardGroup cols={2}>
  ...
  <Card title="Import a post history" href="/bulk-import" icon="download">
    Thousands of posts in one request, replies included, walked in parallel and remembered.
  </Card>
