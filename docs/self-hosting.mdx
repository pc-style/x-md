...
Optionally set `VITE_POSTHOG_KEY` to the same project token and `VITE_POSTHOG_HOST` to its ingestion host in Vercel's production environment. These are public build-time values, not personal API credentials. Rebuild after changing them. Landing-page capture requires a production build with `VERCEL_ENV=production`; local development and previews do not send events.

PostHog capture on the landing page sends only `conversion_requested` and `skill_install_command_copied`, with a new anonymous ID for each event. It does not collect form contents, URLs, referrers, cookies, automatic pageviews, errors, or session recordings. Browser requests go directly to PostHog, which necessarily receives the connection's source IP; event properties disable IP-based enrichment. These action counts are separate from API request totals and cannot measure unique visitors. Existing Vercel Web Analytics on the public landing page is unchanged. The admin dashboard includes no analytics scripts.
