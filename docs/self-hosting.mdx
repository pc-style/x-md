...
Profiles, posts, and bulk imports read `https://api.fxtwitter.com` by default. Set `FXTWITTER_BASE_URL` to point x.md at your own [FxEmbed](https://github.com/FxEmbed/FxEmbed) deployment: bulk imports then run against your capacity instead of the public instance's per-IP limit. How high `concurrency` on the import route can go (its maximum is 32) depends on that deployment's capacity and rate-limit configuration; x.md still backs off on any 429 it returns. The limits in `lib/import.ts` (`IMPORT_MAX_POSTS`, `IMPORT_MAX_CONCURRENCY`) and the front-door quota in `lib/quotas.ts` are the only caps; there are no hosted-only ones.
