...
| `retry_after` | on retryable errors with a delay | Seconds to wait. It mirrors `Retry-After` when headers can still be written; a terminal NDJSON error carries it only in the body. |
| `streamed_posts` | on an NDJSON terminal error | Number of post lines already sent before the error line. |
...
| `Retry-After` | `429`, `503`, and partial import `502` | Seconds to wait. Takes precedence over the `RateLimit` reset if the two disagree. |
...
| Code | Title | Meaning | What to do |
| --- | --- | --- | --- |
| `not_found` | Not found | The post or profile is confirmed missing or inaccessible. An ambiguous provider miss is never enough to establish this. | Confirm the post or profile is public and still exists on x.com, then retry. Deleted and protected content is never available. |
...
| `upstream_unavailable` | Upstream profile unavailable | The provider returned an ambiguous profile miss, such as `User not found`; it could not confirm whether the profile exists. | Wait for `Retry-After`, then retry. Do not treat the profile as deleted. |
| `partial_upstream_failure` | Partial upstream failure | A timeline page stayed unavailable after retries and the stream could not finish. `streamed_posts` counts lines already delivered. | Wait for the error body's `retry_after`, then retry the import and deduplicate by post ID. |
...
| `500`, `502` | Yes | Exponential backoff with jitter. Honor `Retry-After` when present. |
