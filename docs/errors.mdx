...
| `retry_after` | on retryable errors with a delay | Seconds to wait, mirroring the `Retry-After` header. |
...
| `partial_upstream_failure` | Partial upstream failure | A timeline page stayed unavailable after retries and the stream could not finish. `streamed_posts` counts lines already delivered. | Wait for `Retry-After`, then retry the import and deduplicate by post ID. |
