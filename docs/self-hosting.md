...
Each session has a **50-call allowance per 15 minutes**, shared across post and user searches. Failed attempts and page walks count. Accounts that return authentication failures are disabled in the current process; rate-limited accounts cool down for 15 minutes. Session health is per instance.
...
These share IP and account counters across instances. Without them, counters fall back to process memory. Account-budget and per-IP account-allowance checks fail closed when the counter store is unavailable, protecting the shared pool. The general request limiter still fails open. Redeploy after connecting environment variables.
