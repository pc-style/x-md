...
One URL, no key, no OAuth: point your client at `https://x.pcstyle.dev/mcp` and the six tools below show up. Pick your client.
...
<Tabs>
  <Tab title="Claude Code">
    ...
    Then ask Claude Code to read an X post. It picks `x_md_get_post` on its own.
  </Tab>
  <Tab title="Codex">
    ```bash
    codex mcp add x-md --url https://x.pcstyle.dev/mcp
    ```

    Codex CLI, the IDE extension, and the ChatGPT desktop app share this configuration. The equivalent entry in `~/.codex/config.toml`:
  ...
  <Tab title="Amp">
    ```bash
    amp mcp add x-md https://x.pcstyle.dev/mcp
    ```
...
The five `x_md_` tools are read-only and annotated `readOnlyHint: true`, `destructiveHint: false`, `idempotentHint: true`, `openWorldHint: true`. None of them post, reply, like, follow, or read protected accounts, direct messages, or Lists. `submit_feedback` is the one tool that writes, and it writes to the x.md feedback inbox, never to X, so it is annotated `readOnlyHint: false` and `idempotentHint: false`.
...
| `submit_feedback` | The user hits a bug, asks for something x.md does not support, has a question, or wants to say what worked | `message`; `title`, `kind`, `sentiment`, `contextUrl` |

Each read result carries the rendered Markdown as text content plus `structuredContent` matching the tool's `outputSchema`; `submit_feedback` returns a one-line confirmation and the stored entry's `id`. Paging works exactly as it does over HTTP: `limit` caps at 20, `page` caps at 10, and deep walks follow `next_cursor`.
